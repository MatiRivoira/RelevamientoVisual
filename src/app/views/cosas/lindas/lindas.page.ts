...
export class LindasPage implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.firestore.collection('lindas', ref => ref.orderBy('date', 'desc')).snapshotChanges().subscribe(data => {
  ...
  async likeFoto(idFoto:string){
    this.photoService.voteForPhoto(idFoto, "lindas");
    this.userVotes = await this.photoService.loadUserVotes();
  }
